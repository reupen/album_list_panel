			...
			if (ti.flags & TVHT_ONITEM)
			{
				...
				if (p_selection.is_valid())
				{
					...
					if (pDataObject)
					{
						...
						auto icon = icon_ptr((static_api_ptr_t<ui_control>()->load_main_icon(256, 256)));
						...
						uih::CreateDragImage(wnd_tv, colours.get_themed(), m_dd_theme, colourSelectionBackground, colourSelectionText, icon, &lf, showText, text, &sdi);
